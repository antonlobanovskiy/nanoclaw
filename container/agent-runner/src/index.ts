...
/**
 * Stash a message back into IPC input so it's picked up on the next drain.
 */
function stashIpcMessage(text: string): void {
  fs.mkdirSync(IPC_INPUT_DIR, { recursive: true });
  const filename = `stash_${Date.now()}.json`;
  fs.writeFileSync(path.join(IPC_INPUT_DIR, filename), JSON.stringify({ type: 'message', text }));
  log(`Stashed IPC message for next turn (${filename})`);
}
